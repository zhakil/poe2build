...
function New-PyInstallerSpec {
    ...
    # 添加Windows集成相关文件
    $windowsIntegrationFiles = @(
        "src/poe2build/gui/services/windows_integration.py",
        "assets/icons/*.ico",
        "assets/icons/*.png", 
        "resources/templates/*.nsi"
    )
    
    foreach ($pattern in $windowsIntegrationFiles) {
        $files = Get-ChildItem -Path $pattern -ErrorAction SilentlyContinue
        foreach ($file in $files) {
            $relativePath = $file.FullName | Resolve-Path -Relative
            $dataFiles += "('$relativePath', '$(Split-Path $relativePath -Parent)', 'DATA')"
        }
    }
...
; PoE2Build文件关联
[HKEY_CLASSES_ROOT\.poe2build]
@="PoE2Build.BuildFile"

[HKEY_CLASSES_ROOT\PoE2Build.BuildFile]
@="PoE2构筑文件"

[HKEY_CLASSES_ROOT\PoE2Build.BuildFile\DefaultIcon]
@="$BuildDir\\PoE2Build.exe,0"

[HKEY_CLASSES_ROOT\PoE2Build.BuildFile\shell\open\command]
@="\"$BuildDir\\PoE2Build.exe\" --open \"%1\""
"@
        
        $regPath = Join-Path $BuildDir "install_file_association.reg"
        Set-Content -Path $regPath -Value $regScript -Encoding UTF8
        Write-Success "Windows集成脚本生成完成: install_file_association.reg"
...
"@
        ...
        $unregPath = Join-Path $BuildDir "uninstall_file_association.reg"
        Set-Content -Path $unregPath -Value $unregScript -Encoding UTF8
        Write-Success "Windows卸载脚本生成完成: uninstall_file_association.reg"
...
## 文件关联安装
1. 以管理员身份运行命令提示符
2. 双击 install_file_association.reg 文件
3. 确认注册表修改

## 文件关联卸载
1. 双击 uninstall_file_association.reg 文件
2. 确认注册表修改

## 自动启动
应用程序支持自动启动功能，可在设置中启用或禁用。

## 系统托盘
应用程序支持最小化到系统托盘运行，可通过托盘图标访问快速功能。
"@
        
        $instructionsPath = Join-Path $BuildDir "Windows集成安装说明.md"
        Set-Content -Path $instructionsPath -Value $installInstructions -Encoding UTF8
        Write-Success "安装说明文档生成完成"
        
    }
    catch {
        Write-Warning "Windows集成设置失败: $($_.Exception.Message)"
    }
}
...
try {
    ...
    # 6. 构建后处理
    Initialize-WindowsIntegration -BuildDir $OutputDir
