...
class PoB2Calculator:
    ...
    def __init__(self, cache_manager: Optional[CacheManager] = None):
        ...
        if cache_manager:
            ...
            # 创建默认的缓存管理器配置
            from ..resilience.cache_manager import CacheConfig
            import tempfile
            from pathlib import Path
            cache_dir_path = Path(tempfile.gettempdir()) / "poe2build_cache" / "pob2_calculator"
...
class PoB2CalculatorFallback:
    ...
    def calculate_build_stats(self, build_data: dict) -> dict:
        """
        简化的构筑统计计算
        ...
        try:
            ...
            character_class = build_data.get('class', build_data.get('character_class', 'Templar'))
            
            # 简化的DPS估算
            estimated_dps = self._estimate_dps(build_data)
            
            # 简化的生命值估算
            estimated_life = self._estimate_life(build_data, level)
            
            return {
                'calculation_engine': 'Fallback',
                'success': True,
                'total_dps': estimated_dps,
                'life': estimated_life,
                'effective_health': estimated_life,
                'resistances': {
                    'fire': 75,  # 假设抗性
                    'cold': 75,
                    'lightning': 75,
                ...
                'warning': 'Fallback calculation - results are estimates'
            }
    ...
    def _estimate_dps(self, build_data: dict) -> float:
        """估算DPS"""
        base_dps = 50000  # 基础DPS
        level = build_data.get('level', 90)
        
        # 等级缩放
        level_multiplier = 1 + (level - 1) * 0.02
        
        return base_dps * level_multiplier
    
    def _estimate_life(self, build_data: dict, level: int) -> float:
        """估算生命值"""
        base_life = 500  # 基础生命值
        level_life = level * 12  # 每级12点生命值
        
        return base_life + level_life
    
    def validate_build(self, build_data: dict) -> dict:
        """基础构筑验证"""
        try:
            from ..utils.data_validation import PoE2DataValidator
            
            validation_result = PoE2DataValidator.validate_build_data(build_data)
            ...
            # 简化的验证逻辑作为备用
            if isinstance(build_data, dict) and len(build_data) > 0:
                return {'valid': True}
            else:
                return {'valid': False, 'errors': ['Invalid build data structure']}
    
    def get_build_import_code(self, build_data: dict) -> str:
        """备用实现不支持导入代码生成"""
        raise NotImplementedError("Import code generation not available in fallback mode")